...
int main(){
  ...
  ssize_t lenval = read(pp.get(),reinterpret_cast<char*>(&value)+lenread,target-lenread);
  if (lenval==0) value=0;
  else if(lenval!=sizeof(value))return 1;
  lenread =lenread+lenval;
  
  value=value+1;
  target = sizeof(value);
   while(lenwrite!=static_cast<ssize_t>(sizeof(value))){
    ssize_t lenval = write(pp.get(),reinterpret_cast<char *>(&value)+lenwrite,target-lenwrite);
    if (lenval<=0)return 1;
